...
export default function SimulateOrderModal({
  ...
  const addProduct = (productId: string, name: string, price: number, stock: number) => {
    ...
    setCart((prev) => {
      ...
      if (existing) {
        const max = stock > 0 ? stock : existing.quantity + 1;
        return prev.map((line) =>
          line.productId === productId
            ? { ...line, quantity: Math.min(line.quantity + 1, max) }
            : line
        );
      }
      return [
        ...prev,
        { productId, name, price, quantity: 1, stock, notes: "" },
      ];
    });
  };
  ...
  const handleDispatch = async () => {
    if (cart.length === 0) {
      setFormError("Adicione ao menos um item para simular a venda.");
      return;
    }
    ...
    try {
      ...
      if (error) {
        setFormError(
          error.message?.includes("not authorized")
            ...
            : "Não foi possível enviar a simulação."
        );
        return;
      }
      ...
      onDispatched(
        status === "duplicate"
          ? "Esta venda já havia sido lançada."
          : "Pedido da maquininha enviado para o balcão!"
      );
      onClose();
    } catch {
      setFormError("Não foi possível enviar a simulação.");
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <header className="flex items-start justify-between gap-3 border-b border-background-300/60 px-5 py-4">
          <div>
            ...
            <h2 className="mt-0.5 font-heading text-xl font-medium text-foreground-950">
              Simular venda da Moderninha Smart 2
            </h2>
            <p className="mt-0.5 text-xs text-foreground-500">
              Monte uma comanda como a maquininha enviaria e veja o balcão reagir ao vivo.
            </p>
          </div>
          <button
            type="button"
                    ...
                    >
                      ...
                      <span className="mt-1 flex w-full items-center justify-between">
                        ...
                        <span className="text-[10px] uppercase tracking-wider text-foreground-400">
                          {isOut ? "Esgotado" : `${product.current_stock} ${product.unit}`}
                        </span>
                      </span>
                    </button>
                  );
              ...
              {cart.length === 0 ? (
                <div className="flex flex-col items-center gap-2 rounded-md border border-dashed border-background-300 px-4 py-10 text-center">
                  ...
                  <p className="text-sm text-foreground-500">
                    Toque nos itens do cardápio para montar a comanda.
                  </p>
                </div>
              ) : (
